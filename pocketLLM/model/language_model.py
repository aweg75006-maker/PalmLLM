...
# 反向传播
# 计算损失函数关于每个参数的梯度（偏置也是），每个参数都向着梯度下降的方向变化一点点，来实现损失值最小的过程，这就构成了神经网络的训练过程。下面是详细讲解：
...
# 因此需要动态调整步长，即学习率，让步长随着梯度变化，如果梯度很大，步长就大，如果梯度很小，步长就小。（替换固定学习率为余弦退火）
...
class LanguageModel(nn.Module):
    def __init__(self, cfg):
        ...
        Args:
            cfg (dict): 配置字典，包含以下字段：
                ...
                "num_heads": 多头注意力中的注意力头的数量 每个头的维度是64维度 head_dim = d_out // num_heads = emb_dim // num_heads
    ...
    def forward(self, token_ids):
        ...
        # 模型直接输出Logits (原始分数),形状 (batch, tokens, vocab_size)
        # 模型每一步生成的内容都是从词表 vocab_size 对应的 token 集合里选出来的
        # 所以大模型生成文本的本质是：基于当前上下文，预测词表中哪个 token 最可能作为下一个 token
        # 模型不能直接输出词表里不存在的 token。但是它可以通过多个 token 拼出新的词
        # 非文本是数值张量
        # 最后一步调用 tokenizer.decode(token_ids) 成为文本输出
        # Logits → 选 Token ID → 追加到序列 → 循环 → 全部解码为文本
        return logits
