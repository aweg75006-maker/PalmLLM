...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # 为什么转置
            ...
            # 批次大小 = 2, token数 = 5, 头数 = 12, 每头维度 = 64
            # 理解为: 对每个token, 有12个头的表示
        # 转置后
            # 形状: (batch_size, num_heads, num_tokens, head_dim)
            # 批次大小 = 2, 头数 = 12, token数 = 5, 每头维度 = 64
            # 理解为: 对每个注意力头, 处理所有token
        # 总结: transpose(1, 2) 就是交换张量的第1维和第2维,在多头注意力中用于将数据从"按token组织"转换为"按注意力头组织",以便并行计算。
    ...
    # 计算注意力分数
        ...
        attn_scores = queries @ keys.transpose(2, 3) # @ 表示矩阵乘法
