...
from trl import DPOTrainer # DPO 损失函数
...
def get_dataset(split: str, file: str, cache_dir: str = '.cache') -> Dataset:
    ...
    """
    # 这段代码就是专门为 DPO (Direct Preference Optimization) 算法处理标准数据集
    ...
    def split_prompt_and_responses(sample: dict) -> Dict[str, str]:
        return {
            ...
            "prompt": f"{sample['prompt']}[EOS]", # 用户的问题 / 指令（DPO 的输入）
            "chosen": f"{sample['chosen']}[EOS]", # 好回答（DPO 要让模型学习生成这个）
            "rejected": f"{sample['rejected']}[EOS]", # 坏回答（DPO 要让模型避免生成这个）
            # EOS句子结束符，DPO 训练 / 生成时必须加，用来标记文本终止
        }

    return dataset.map(split_prompt_and_responses).shuffle(2333)


def train_dpo(config: DpoConfig, peft_config: LoraConfig=None) -> None: # 没有lora
    ...
    dpo_trainer = DPOTrainer(
        model_train, # SFT 之后的模型
        model_ref, # 参考模型（通常是模型的副本，防止模型跑偏）
        peft_config=peft_config,
        args=training_args,
        beta=config.beta, # ← KL 散度惩罚系数，控制偏好程度
